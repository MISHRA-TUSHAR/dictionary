...
class DataController extends GetxController {
  ...
  Future<List<Map<String, String>>> readTsvFromAssets() async {
    ...
    String fileContent =
        await rootBundle.loadString('assets/english_to_hindi.tsv');
    List<List<dynamic>> csvTable =
        const CsvToListConverter(fieldDelimiter: '\t').convert(fileContent);
  ...
  void binarySearch(String searchText) {
    ...
    for (int i = 0; i < dictionary.length; i++) {
      ...
      if (dictionary[i]['english'] != null) {
        if (dictionary[i]['english']!.toLowerCase() ==
        ...
        if (dictionary[i]['english']!
                .toLowerCase()
                .contains(searchText.toLowerCase()) &&
            searchedword.length < 12 &&
            dictionary[i]['english']!.toLowerCase() !=
                searchText.toLowerCase()) {
